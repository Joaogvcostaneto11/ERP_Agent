...
Bill ingestion requires `ANTHROPIC_API_KEY`, `DATABASE_URL` (read-only, for lookups), `BILLS_WRITE_DATABASE_URL` (a writable login scoped to the ForumSI mirror), and the Tesseract + Poppler binaries (`tesseract-ocr`, `tesseract-ocr-por`, `poppler-utils`). QR decoding additionally needs `opencv-python-headless`. Set `BILLS_APP_PASSWORD` to put the service behind an HTTP Basic gate — unset (local dev) leaves it **ungated**, which is fine on a laptop and unacceptable anywhere reachable, since this service writes to the database. It is deliberately a different secret from the chat service's `APP_PASSWORD`. Before first deploy, run `db/migrations/002_bills_audit.sql` against the write database: the audit row is inserted in the same transaction as the document, so a missing table fails every commit. Accepted bills are written as draft, uncertified `Doc001` documents.
