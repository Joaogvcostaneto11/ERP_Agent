...
# bills_examples/ is untracked, and the decoder (opencv-python-headless) is an
# optional dependency: both skip rather than fail where they are absent. Only
# the tests that actually need a fixture file AND a real decode carry this —
# the self-contained memory/guard tests below must run everywhere.
requires_fixtures_and_decoder = pytest.mark.skipif(
    not EXAMPLES.is_dir() or importlib.util.find_spec("cv2") is None,
    reason="bills_examples/ not present or decoder (cv2) not installed")
...
def test_decode_returns_none_when_the_decoder_import_raises_a_non_import_error(monkeypatch):
    # A numpy/OpenCV ABI mismatch (the classic failure for a floored-not-pinned
    # OpenCV version) surfaces as AttributeError or RuntimeError on `import
    # cv2`, not ImportError. decode()'s contract is to never raise, so the
    # guard around the import must catch this too, not just ImportError.
    # Self-contained: it forces the failure itself, so it needs neither the
    # fixture files nor a real decoder to be installed.
    import builtins
    real_import = builtins.__import__

    def _broken_cv2(name, *args, **kwargs):
        if name == "cv2":
            raise AttributeError("numpy ABI mismatch (simulated)")
        return real_import(name, *args, **kwargs)

    monkeypatch.setattr(builtins, "__import__", _broken_cv2)
    assert decode(b"not an image at all") is None
